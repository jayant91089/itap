...
#! To get the latest version of this GAP 4 package download one of the
#! archive files  itap-x.x.zip, itap-x.x.tar.gz, itap-x.x.zoo,  itap-x.x.tar.bz2
#! from <URL>https://github.com/jayant91089/itap</URL> or <URL>http://www.ece.drexel.edu/walsh/aspitrg/software.html</URL>
#! and unpack it using
...
#! @InsertCode Read
#! from within GAP.
#! $\textbf{Optional Dependencies}$: $\texttt{itap}$ can optionally use gap package $\texttt{FinInG}$.
#! $\texttt{FinInG}$ provides allows internal functions to use projective semilinear group $P\Gamma L(k,q)$,
#! which is the group of all collineations  of the vector space $GF(q)^k$.
#! See <URL>http://cage.ugent.be/fining/</URL> for information on how to obtain and install this package.
#! $\texttt{LoadPackage( "itap")}$ automatically checks if $\texttt{FinInG}$ is available and
#! loads it in case it is available. If $\texttt{FinInG}$ is absent, a light version of
#! $\texttt{itap}$ is loaded, where the internal functions default to using projective general linear group
#! $PGL(k,q)\leq P\Gamma L(k,q)$. Note that this distinction is irrelevent if $q$ is a prime number
#! as $PGL(k,q)\cong P\Gamma L(k,q)$.
#! @BeginCode GF
q:=4;;
...
DeclareGlobalFunction("findrep2");
